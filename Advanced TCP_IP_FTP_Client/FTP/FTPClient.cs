...
namespace Advanced_TCP_IP_FTP_Client
{
    ...
    class FTPClient
    {
        public static event ClientLogHandler ClientLogEvent;
        ...
        private static void Connect(string ip, string username, string password)
        {
            ...
            NewClientLog($@"Connected after {connectionAttempts} attempts");
            string response = string.Empty;
            if (!response.StartsWith("220"))
            {
                NewClientLog("Server is not accepting clients");
        ...
        private static string SendCommand(string command)
        {
            ...
            try
            {
                NewClientLog("Sending: " + command);
                command += "\r\n";
                _ClientSocket.Send(Encoding.ASCII.GetBytes(command));
                response = AwaitResponse();
            }
            catch (SocketException ex)
            { NewClientLog("SendCommand " + ex.Message); }
        ...
        private static string AwaitResponse()
        {
            ...
            try
            {
                ...
                NewClientLog($@"Received: {Encoding.ASCII.GetString(receiveddata)}");
            }
            catch (SocketException ex)
            { NewClientLog("AwaitResponse " + ex.Message); }
            catch (ObjectDisposedException ex)
            { NewClientLog("AwaitResponse " + ex.Message); }
            return response;
        }
        ...
        {
            ClientLogArgs args = new ClientLogArgs { Message = message };
            OnNewClientLog(args);
        }
