...
namespace Simple_TCP_IP_FTP_Server
{
    ...
    class FTPClient
    {
        ...
        //Permissions
        private bool _CanViewHiddenFiles = true;
        private bool _CanViewHiddenFolders = true;
        private bool _CanStoreFiles = true;
        private bool _CanStoreFolders = true;
        private bool _CanRenameFiles = true;
        private bool _CanRenameFolders = true;
        private bool _CanDeleteFiles = true;
        private bool _CanDeleteFolders = true;
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
            ...
            switch (Command)
            {
                ...
                case "PASS":
                    if (_Username == "") { SendMessage("503 Invalid User Name\r\n"); return; }

                    string correctPass;
                    Program.UserList.TryGetValue(_Username, out correctPass);

                    if (correctPass == CmdArguments) { _IsAuthenticated = true; SendMessage("230 Authentication Successful\r\n"); Program.StartingDirList.TryGetValue(_Username, out _StartingDir); }
            ...
            if (!CmdExecuted)
            {
                if (_IsAuthenticated)
                {
                    ...
                    switch (Command)
                    {
                        ...
                        case "PWD": SendMessage("257 \"" + _WorkingDir.Replace('\\', '/') + "\"\r\n"); break;
                        //Disconnect.
        ...
        {
            string[] _Pathparts = _WorkingDir.Split('\\');
            if (_Pathparts.Length > 1)
            {
                _WorkingDir = "";
                for (int i = 0; i < (_Pathparts.Length - 2); i++)
                {
                    _WorkingDir += _Pathparts[i] + "\\";
                }
            }
        ...
        private void DELE(string CmdArguments)
        {
            ...
            Path = _StartingDir + Path.Substring(0, Path.Length - 1);
            try
            {
                if (File.Exists(Path))
                {
        ...
        {
            string Path = _StartingDir + GetExactPath(CmdArguments);
            if (!_CanViewHiddenFolders && (new DirectoryInfo(Path).Attributes & FileAttributes.Hidden) == FileAttributes.Hidden)
            {
        ...
        private void RETR(string CmdArguments)
        {
            ...
            try
            {
                ...
                if (!_CanViewHiddenFiles && (File.GetAttributes(Path) & FileAttributes.Hidden) == FileAttributes.Hidden)
                {
        ...
        {
            if (!_CanRenameFiles) { SendMessage("550 Access Denied.\r\n"); return; }
        ...
        private void RNTO(string CmdArguments)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        if (_CanRenameFolders) { Directory.Move(_RenameFP, Path); SendMessage("250 Folder renamed successfully.\r\n"); }
                        else SendMessage("550 Access Denied.\r\n");
                    }
                    else if (File.Exists(RenameFile))
                    {
                        if (_CanRenameFiles) { File.Move(RenameFile, Path); SendMessage("250 File renamed successfully.\r\n"); }
                        else SendMessage("550 Access Denied.\r\n");
                    }
        ...
        private void Disconnect()
        {
            ...
            Program.FTPClientsList.Remove(this);
            GC.Collect();
        }
        ...
        string GetExactPath(string Path)
        {
            ...
            if (!Path.StartsWith("/")) _Dir = _WorkingDir + _Dir;
